...
class LampiApp(App):
    ...
    my_process  = None
    # AWS credentials
    AWS_ACCESS_KEY_ID = 'AKIAUC7NAIENYR4HPU4X'
    AWS_SECRET_ACCESS_KEY = '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'

    # S3 bucket info
    BUCKET_NAME = 'chattabox'
    BUCKET_FOLDER = 'lampi_to_iphone/'
    ...
    def start_recording(self):
        content = Label(text='Recording')
        stop_button = Button(text='End', size_hint_y=0.05, size_hint_x=0.05)
        popup = Popup(title='RECORD', content=content, size_hint=(0.8, 0.4), pos_hint={'top': 1}, auto_dismiss=False)
        content.add_widget(stop_button)
        stop_button.bind(on_press=popup.dismiss)
        popup.open()
        print('recording')
        self.my_process = subprocess.Popen(['arecord', '-D', 'plughw:2,0', '-f', 'cd', 'lampi_recording.wav'])

        def stop_recording():
            self.my_process.send_signal(signal.SIGINT)
            print('stopped')

        popup.on_dismiss = stop_recording

    def on_send(self):
        # upload the recording to S3 
        popup = Popup(title='RECORD', content=Label(text='Sent'), size_hint=(0.8, 0.4), pos_hint={'top': 1}, auto_dismiss=True)     
        popup.open()
        s3_client = boto3.client('s3', aws_access_key_id=self.AWS_ACCESS_KEY_ID, aws_secret_access_key=self.AWS_SECRET_ACCESS_KEY)
        s3_client.upload_file('lampi_recording.wav', self.BUCKET_NAME,'lampi_recording.wav')

    def on_playback(self):
        subprocess.call(['aplay','-Dhw:1,0', 'lampi_recording.wav'])

    def on_play_new_message(self):
        subprocess.call(['aplay','-Dhw:1,0', './phone_recording.wav'])